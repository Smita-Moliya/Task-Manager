...
def get_me_attachments(role: str, user_id: int) -> List[Dict[str, Any]]:
    rows = me_db.me_attachments_admin() if role == "ADMIN" else me_db.me_attachments_user(user_id)
    out = []
...
def get_me_comments(role: str, user_id: int) -> List[Dict[str, Any]]:
    rows = me_db.me_comments_admin() if role == "ADMIN" else me_db.me_comments_user(user_id)
    out = []
